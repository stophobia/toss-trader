...
토스증권 Open API + `oh-my-openagent` 기반 투자 어시스턴트. paper trading 기본값, 실계좌는 명시적 opt-in.
...
4. **yolo 모드 금지** — 원본 `codex --yolo` 패턴은 oh-my-openagent 화이트리스트로 대체
5. **서버 만들지 마** — CLI + Telegram inline button 구조 유지. Vercel/Next.js 이관 ❌

## 핵심 기술 결정

- **언어**: Python 3.11+ (PEP 604 union 필수)
- **분석 엔진**: `oh-my-openagent` (sigil `~/.local/bin/oh-my-openagent`)
...
# 3) oh-my-openagent 확인
which oh-my-openagent || echo "❌ sigil 필요"
...
- oh-my-openagent 사용법: `oh-my-openagent --help`
...
- 4단계: `agent/openagent.py` oh-my-openagent 래퍼
